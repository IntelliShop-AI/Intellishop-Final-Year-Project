...
const OrderScreen = ({ route, navigation }) => {
  ...
  // Define state to hold recommended image URLs
  const [recommendedImageUrls, setRecommendedImageUrls] = useState([]);

  //defining necessary useState Hooks
  ...
  const confirmOrder = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "http://192.168.1.194:6000/confirm_order",
        {
          userId: firebase.auth().currentUser.uid,
          productIds: productSummary.map((product) => product.id),
        }
      );

      // Extract recommended image URLs from the response
      const recommendedImageUrls = response.data.recommended_image_urls;
      console.log(recommendedImageUrls);

      // Update state with recommended image URLs
      setRecommendedImageUrls(recommendedImageUrls);

      // Navigate back to Home Screen and pass recommended image URLs as params
      navigation.navigate("Home2", {
        recommendedImageUrls: recommendedImageUrls,
      });
    } catch (error) {
